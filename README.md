# The Dungeon

Boss battle dungeon game.

React/Vite turn-based dungeon game with JavaScript combat logic, class-based hero selection, randomized damage,
critical-hit mechanics, and escalating boss encounters. Migrated from an original Java console game into a responsive
browser app with reusable game data, state-driven battle flow, and modular UI components.

## Features
- 5 playable hero classes
- 5 escalating boss encounters
- Turn-based ability combat
- Randomized damage, misses, and critical hits
- Health carryover between fights
- Victory, defeat, and final dungeon clear screens

## Technologies
- JavaScript
- React
- Vite
- CSS
- Java

## Installation
cd web
npm install

## Run
npm run dev

## Screenshots

### Start Screen

![Start Screen](screenshots/start-screen.png)

### Class Select

![Class Select](screenshots/class-select.png)

### Boss Matchup

![Boss Matchup](screenshots/boss-matchup.png)

### Battle Screen

![Battle Screen](screenshots/battle-screen.png)

### Victory Screen

![Victory Screen](screenshots/victory-screen.png)
